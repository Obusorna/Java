RightNow.namespace('Custom.Widgets.pnoc.customSubmitPNOC');
Custom.Widgets.pnoc.customSubmitPNOC = RightNow.Form.extend({
    overrides: {
	...
	_onBeforeAjaxRequest: function(type, args)
	{
        ...
        var lastPart = url.substr(url.lastIndexOf('app/PNOC_p2/') + 12,24);
		
		if(lastPart=="createcircuitrequesttest") {
			if(requestOptions.url == "/ci/ajaxRequest/sendForm") {
		    
            requestOptions.url = "/cc/AjaxCustom/changeDate";
			}
		}
	},
	...
	*/
    _onButtonClick: function(evt) {
		RightNow.Event.fire("evt_checkCustomFields");
        if (evt && evt.halt) {
            evt.halt();
		}
    ...
    _formSubmitResponse: function(type, args) {
		...
		if (!responseObject) {
            ...
            if (result.sa) {
				...
				//----------------- custom code by kamalpreet starts here -------------------------
				var action = "create";
				var email_ids = "";
				var chk_email_pnoc = document.getElementsByName("chk_email_pnoc");
				if(chk_email_pnoc.length)
				{
					action = "update";
					var arr =new Array();
					var i;
					var k = 0;
					for (i = 0; i < chk_email_pnoc.length; i++) 
					{
						if (chk_email_pnoc[i].checked) 
						{
							arr[k] = chk_email_pnoc[i].value;
							k++;
						}
					}
					email_ids = arr.join(',');
				...
				if(action == "create")
				{
					var eo = new RightNow.Event.EventObject(this, {data:{
						...
						i_id: responseObject.result.transaction.incident.value,
					}});
					
					RightNow.Ajax.makeRequest('/cc/AjaxCustom/insertCustomReferenceID', eo.data, {
				...
				{
					var status_element = document.getElementsByName("Incident.CustomFields.c.custom_status");
					var id_status = status_element[0].getAttribute( 'id' ); 
					var custom_status = document.getElementById(id_status).value;
					...
					RightNow.Ajax.makeRequest('/cc/AjaxCustom/sendEmail', eo.data, {
						successHandler: this._Show,
						scope:          this,
						data:           eo,
						json:           true
					});	
